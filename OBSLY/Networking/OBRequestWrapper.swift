...
class OBRequestWrapper: NSObject {
    ...
    let defaultKey = "hZXTt8eh7PGBA9ufMVRZr1uwTnKhCU6_2XjIFflBFXk="
    ...
    func getCurrenciesWithCompletionBlock(completion: @escaping (_ responseObject: Any) -> Void)  {
        let urlPath: String = "\(self.backendPath)\(defaultKey)/currencies"
...
//                    }
    ...
    func getRate(fromCurr: String, toCurr: String, completion: @escaping (_ responseObject: Any) -> Void)  {
        let defaults = UserDefaults.standard
        var chosenBookKey = ""
        if let chosenBookKeyFromDefaults = defaults.string(forKey: "chosenBookKey") {
            chosenBookKey = chosenBookKeyFromDefaults
        } else {
            chosenBookKey = defaultKey
        }
        let urlPath: String = "\(self.backendPath)\(chosenBookKey)/currencies/rate?currency_from=\(fromCurr)&currency_to=\(toCurr)"
        let url: NSURL = NSURL(string: urlPath)!
        let request: NSMutableURLRequest = NSMutableURLRequest(url: url as URL)
        request.httpMethod = "GET"
        let session = URLSession.shared
